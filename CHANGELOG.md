...
- **Adapter `getLatestVersion()` honesty**: built-in adapters no longer fabricate a "latest" version. Outside test mode, a failed or unparseable lookup returns `latestVersion: null` ("unknown") instead of a pinned constant (claude-code `2.1.237`, cline `3.5.0`, codex `1.2.0`, kilo `0.9.4`, pi `1.2.0`), so the manager's 60 s failure TTL and `isOutdated(installed, null) === false` now engage and the Web Deck/CLI stop showing a stale "latest" or a false "outdated"/"up to date". Hermes (`0.20.0`) and OpenClaw (`2026.7.1-2`) returned constants with no lookup at all; they now query GitHub Releases (`NousResearch/hermes-agent`) and the npm registry (`openclaw`). GarraIA's offline fallback reported the installed version or the literal `'unknown'` as "latest"; it now returns `null`. npm lookups share `fetchLatestNpmVersion()` (checks the exit code, validates the output, 15 s timeout). Consequence: `agentdeck upgrade <agent>` without a target now fails clearly when offline instead of planning an upgrade to a phantom version.
- **`agentdeck agents setup --per-agent` now applies the per-agent choice**: the override loop prompted for a provider/model per agent and printed "X usará …", but the deck routing was then written to every agent anyway. `RoutingService.applyToAgents` accepts `overrides` (per adapter id) used for both the pre-write dry-run gate and the apply; the run's manifest records the routing each agent received, the report exposes `overrides`, and the CLI summary shows each agent's effective provider/model (tagged `(próprio)` for overrides). An override on a new provider goes through the same key verification and model check as the deck primary, and a rejected key aborts before anything is persisted. `--per-agent` without an interactive terminal (or with `--yes`) now fails fast instead of silently applying the deck routing.
- **Web Deck auth dialog**: the "Authentication Required" dialog (`--token` mode) now renders above and after every other modal (`z-[60]`, last in DOM order), so a 401 raised from inside the persona/instance/room dialogs no longer leaves it hidden behind them; it also traps keyboard focus (Tab/Shift+Tab cycle inside, Escape does not dismiss it) and returns focus to the previously focused element when it closes.
