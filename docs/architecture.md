...
`agentdeck mcp-server` (`apps/cli/src/mcp-server.ts`) exposes the deck over MCP stdio with the tools `agentdeck_list_agents`, `agentdeck_ask`, `agentdeck_room_post` and `agentdeck_room_history`; `agentdeck agents link` registers it (and `garra mcp-server`) in each MCP-capable agent. Every `ask` passes `packages/core/src/interop-guardrails.ts` (`room_post` only its rate limiter and fan-out cap): room membership is the allowlist, cycles are detected by call path, and `DEFAULT_INTEROP_LIMITS` cap depth at 3, turns per conversation at 12, conversation time at 600 s, fan-out at 4, and calls at 30/min. Refusals are returned as structured tool errors, never thrown, and every exchange is recorded in a room.
