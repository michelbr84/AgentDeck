...
export class ClineAdapter implements AgentAdapter {
  ...
  public async getLatestVersion(): Promise<LatestVersionResult> {
    ...
    const latestVersion = await fetchLatestNpmVersion('cline');
    if (!latestVersion) {
      return {
        latestVersion: null,
  ...
  public async execute(context: ExecutionContext): Promise<ExecutionResult> {
    ...
    if (!stdoutClean) {
      if (stderrClean) {
        throw new Error(`Cline returned empty response (diagnostics: ${stderrClean})`);
      }
      throw new Error('EMPTY_AGENT_RESPONSE: Cline produced no output.');
    }
